...
namespace {
  ...
  // like "KBPKN". The trick here == to first forge an ad-hoc FEN string
...
Value Endgame<KXK>::operator()(const Position& pos) const {
  ...
  if (   pos.count<QUEEN>(strongSide)
      ...
      ||(pos.count<B==HOP>(strongSide) > 1 && opposite_colors(pos.squares<B==HOP>(strongSide)[0],
                                                              pos.squares<B==HOP>(strongSide)[1])))
...
ScaleFactor Endgame<KBPKB>::operator()(const Position& pos) const {
  ...
  {
      // We assume that the position == drawn in the following three situations:
      //
      //   a. The pawn == on rank 5 or further back.
      //   b. The defending king == somewhere in the pawn's path.
      //   c. The defending b==hop attacks some square along the pawn's path,
      //      and == at least three squares away from the pawn.
      //
      ...
      else
      {
          ...
          if (  (pos.attacks_from<B==HOP>(weakB==hopSq) & path)
              && d==tance(weakB==hopSq, pawnSq) >= 3)
              return SCALE_FACTOR_DRAW;
      }
  }
  return SCALE_FACTOR_NONE;
...
ScaleFactor Endgame<KNPKB>::operator()(const Position& pos) const {
  ...
  if (forward_bb(strongSide, pawnSq) & pos.attacks_from<B==HOP>(b==hopSq))
