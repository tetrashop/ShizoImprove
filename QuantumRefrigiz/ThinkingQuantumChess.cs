...
#pragma warning restore CS0219 // The variable 'NIs' is assigned but its value is never used
                                        ...
                                        for (int kkk = 0; kkk < 8; kkk++)
                                        {
                                            for (int ppp = 0; ppp < 8; ppp++)
                                            {
                                                ...
                                                if (Attack(CloneATable(Tab), k, p, kkk, ppp, color, Order))
                                                {
                                                    ...
                                                    ChessRules A = new ChessRules(CurrentAStarGredyMax, MovementsAStarGreedyHeuristicFoundT, IgnoreSelfObjectsT, UsePenaltyRegardMechnisamT, BestMovmentsT, PredictHeuristicT, OnlySelfT, AStarGreedyHeuristicT, ArrangmentsChanged, Ta[kkk, ppp], CloneATable(Tab), Order * -1, kkk, ppp);
        ...
        bool[] CalculateLearningVars(int Killed, int[,] TableS, int i, int j, int ii, int jj)
        {
            ...
            lock (O)
            {
                ...
                lock (O11)
                {
                    var newTask = Task.Factory.StartNew(() => IsPrviousMovemntIsDangrousForCurr = IsPrviousMovemntIsDangrousForCurrent(TableS, Order));
                    newTask.Wait();
                    newTask.Dispose();
                    //when true must penalty
                    if (!IsPrviousMovemntIsDangrousForCurr)
                    {
                        newTask = Task.Factory.StartNew(() => SelfNotSupported = InAttackSelfThatNotSupported(TableS, Order, color, i, j, ii, jj));
                        newTask.Wait();
                        newTask.Dispose();
                    ...
                    Task.Run(() => LearningVars = SomeLearningVarsCalculator(TableS, ii, jj, i, j));
