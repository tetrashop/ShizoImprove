            ...
            try
            {
                ...
                for (Gen1 = 0; Gen1 < 8; Gen1++)
                    for (Gen2 = 0; Gen2 < 8; Gen2++)
                    {
                        ...
                        if ((new ChessRules(GeneticTable[CromosomRow, CromosomColumn], GeneticTable, Order, CromosomRow, CromosomColumn)).Rules(CromosomRow, CromosomColumn, Gen1,
                        Gen2, color, GeneticTable[CromosomRow, CromosomColumn]))
                        {
                            ...
                            {
                                //Check Consideration.
                                if ((new ChessRules(GeneticTable[CromosomRow, CromosomRow], GeneticTable, Order, CromosomRow, CromosomColumn)).Check(GeneticTable, Order))
                                {
                                    GeneticTable[CromosomRow, CromosomColumn] = GeneticTable[Gen1, Gen2];
                                    GeneticTable[Gen1, Gen2] = 0;
                                    continue;
                ...
                if (Order == 1)
                {
                    ...
                    do
                    {
                        if (Ki < 6)
                            ...
                            Ki = 1;
                        Count++;
                    } while (Count < 6 && !(new ChessRules(Ki, List[List.Count - 1], Order, CromosomRow, CromosomColumn)).FindAThing(List[List.Count - 1], ref CromosomRow, ref CromosomColumn, Ki, true, RowColumn));
                    if (Count >= 6)
                    {
                        NoGameFounf = true;
                        return null;
