...
namespace AppRefregitz
{
    class ChessPerdict
    {
                                    ...
                                    if (SolderesOnTable[i].SoldierThinking[k].ReturnHuristic(i, j, Order) >= Less)
                                    {
                                        ...
                                        else
                                        {
                                            if (AA.CheckBrownObjectDangour && !AllDraw.AStarGreadyFirstSearch)
                                            {
                                                ...
                                                if (Table == null)
                                                    continue;
                                                ChessRules AAA = new ChessRules(1, Table, FormRefrigtz.OrderPlate, -1, -1);
        ...
        public int[,] InitiatePerdictCheck(int ii, int jj, Color a, int[,] Table, int Order, bool TB)
        {
            ...
            AA = new ChessRules(1, TablInitCheck, Order, -1, -1);
            //Check Consideration.
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (iiii < 16)
                    this.InitiateForEveryKindThingHome(new AllDraw(ref THIS), (int)(int)SolderesOnTable[In].Row, (int)(int)SolderesOnTable[In].Column, a, TablInit, Order, false, In);
                else
                    this.InitiateForEveryKindThingHome(new AllDraw(ref THIS), (int)1, 2, a, TablInit, Order, false, In);
