    ...
    public class DrawSoldier : ThingsConverter
    {
            ...
            )
        {
            ...
            for (int i = 0; i < 8; i++)
                ...
                AA = new DrawSoldier(this.Row, this.Column, this.color, Tab, this.Order, false, this.Current);
                
            for (int i = 0; i < AllDraw.SodierMovments; i++)
            {
                try
                {
                    AA.SoldierThinking[i] = new ThinkingChess((int)this.Row, (int)this.Column);
